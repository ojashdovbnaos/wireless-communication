%% Combined MED SOS Channel Analysis: Amplitude, Phase & Time Envelope
clear; clc; close all;
...
%% 2) Generate MED Doppler Frequencies & Random Phases
n        = (1:N).';
theta_n  = 2*pi*(n-0.5)/N;          % equally spaced angles in [0, 2π)
f_n      = fd * cos(theta_n);       % Doppler frequencies [Hz]
phi_n    = 2*pi * rand(N,1);        % random initial phases

%% 3) Construct Complex Envelope h(t) (Vectorized)
exponent = 2*pi*(t * f_n.') + phi_n.';  % [fs*T × N]
h_t      = sum(exp(1j * exponent), 2) / sqrt(N);  % [fs*T × 1], normalized

%% 4) Extract Amplitude & Phase
amplitude = abs(h_t);      % Rayleigh-like envelope
phase     = angle(h_t);    % uniform in [–π, π)
...
ylabel('PDF');
title('(a) MED Amplitude Distribution');
...
ylabel('PDF');
title('(b) MED Phase Distribution');
xlim([-pi, pi]);
ylim([0, 1/(2*pi)*1.5]);
...
title('(c) MED Time-Varying Envelope');
xlim([0, T]);
grid on;
